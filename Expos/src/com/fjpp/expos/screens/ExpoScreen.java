...
public class ExpoScreen {
	...
	// Fetch dogs and expos from the database
	public ArrayList<Dog> getDogsFromDB() {
		...
		String query = "SELECT dog_id, dog_name, dog_breed, dog_birthdate FROM dogsinfo WHERE owner_id = ?";
		try {
	...
	public ExpoScreen(String userId, String userName) {
		...
		lblClass.setBounds(75, 250, 50, 50);
		frame.add(lblClass);
		
		// Fetch dogs from DB
		ArrayList<Dog> dogs = getDogsFromDB();
		dogComboBox = new JComboBox<>(dogs.toArray(new Dog[0]));
		dogComboBox.setBounds(150, 150, 300, 40);
		dogComboBox.setSelectedIndex(-1); // No default selection
		frame.add(dogComboBox);
		
		// Fetch expos from DB
		ArrayList<Expo> expos = getExposFromDB();
		expoComboBox = new JComboBox<>(expos.toArray(new Expo[0]));
		expoComboBox.setBounds(150, 200, 300, 40);
		expoComboBox.setSelectedIndex(-1); // No default selection
		frame.add(expoComboBox);
		
		classComboBox = new JComboBox<>(new String[] {"Baby","Puppy", "Junior", "Intermediate", "Open", "Working", "Champion", "Veteran"});
		classComboBox.setBounds(150, 250, 300, 40);
		classComboBox.setSelectedIndex(-1); // No default selection
